        ...
        <div className="max-w-6xl mx-auto space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              <StatCard icon={Library} color="bg-blue-600" label="Total de Livros" value={totalBooks} />
              <StatCard icon={CheckCircle} color="bg-emerald-500" label="Disponíveis" value={available} />
              <StatCard icon={Users} color="bg-amber-500" label="Emprestados" value={borrowed} />
              <StatCard icon={AlertTriangle} color="bg-red-500" label="Extraviados" value={lost} />
              <StatCard icon={BookOpen} color="bg-purple-500" label="Lidos" value={read} />
              <StatCard icon={Book} color="bg-slate-400" label="Não lidos" value={unread} />
            </div>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              <h3 className="text-sm font-semibold text-slate-800 mb-6">Livros adicionados por mês</h3>
              <div className="h-64">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col">
              <h3 className="text-sm font-semibold text-slate-800 mb-6 flex items-center gap-2">
                <span className="text-[#1a5eb8]">🏷️</span> Categorias mais populares
              </h3>
              ...
              <div className="flex-1 space-y-5">
                {popularCategories.slice(0, 5).map((cat, i) => (
                  <div key={i}>
                    <div className="flex justify-between text-xs font-medium text-slate-700 mb-2">
                      <span>{cat.name}</span>
                      <span className="text-slate-500">{cat.count} livro{cat.count !== 1 ? 's' : ''} · {cat.percent}%</span>
                    </div>
                ...
                {popularCategories.length === 0 && <div className="text-sm text-slate-400">Nenhuma categoria</div>}
              </div>

              <div className="mt-8 pt-6 border-t border-slate-100">
                <div className="flex justify-between text-xs font-medium text-slate-700 mb-2">
                  <span>Progresso de leitura</span>
                  <span className="text-slate-500">{read} de {totalBooks} lidos · {readPercent}%</span>
                </div>
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
          ...
          {/* Por Tema */}
          <div>
            <h3 className="text-lg font-semibold text-slate-800 mb-4">Por Tema</h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              ...
              {books.filter(b => b.readStatus === 'A ler').length === 0 && <div className="col-span-full text-center py-10 text-slate-400">Nenhum livro a ser lido no momento</div>}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              ...
              {books.length === 0 && <div className="col-span-full text-center py-10 text-slate-400">Nenhum livro adicionado</div>}
            </div>
          </div>
